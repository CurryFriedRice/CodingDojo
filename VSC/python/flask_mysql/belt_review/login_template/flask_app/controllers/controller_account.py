#import the app
from VSC.python.flask_mysql.validation.private_wall.flask_app.models.model_account import Account
...
def login():
    user_in_db = MODEL.get_one_with_email({"data" :request.form['email']})
    ...
    print(user_in_db[0].uuid)
    session['uuid'] = user_in_db[0].uuid
...
def create():
    if not Account.validate(request.form):
        return redirect("/account")
    #make and format the data
    form_dict = dict(request.form)
    form_dict['password'] = bcrypt.generate_password_hash(request.form['password'])
    form_dict['uuid'] =  form_dict['id']#bcrypt.generate_password_hash(request.form['id']) 
    #add the entry
...
def update():
    if 'uuid' not in session:
        return redirect("/account")
    user_in_db = Account.get_one_with_uuid({'uuid': session['uuid']})
    if not user_in_db:
