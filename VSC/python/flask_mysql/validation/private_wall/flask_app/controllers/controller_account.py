...
def show_account():
    ...
    context = {
        "accounts" : MODEL.get_one({"id": session['uuid']}),
        "recieved_messages" : model_message.Message.get_one_with_reciever({"reciever_id" : session["uuid"]}),
        "sent_messages" : model_message.Message.get_one_with_sender({"account_id": session['uuid']}),
        "users" :MODEL.get_all()
    }
