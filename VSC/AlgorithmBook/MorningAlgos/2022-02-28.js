...
class BinarySearchTree {
     ...
     * [25, 15, 10, 4, 12, 22, 18, 24, 50, 35, 31, 44, 70, 66, 90] // results
    ...
    {
        if(node == null) return vals;
        if(vals.length == 0) vals.push(node.data);
        else
        {
            let i = 0;
            while(node.data > vals[i])
                i++;
            vals.splice(i,0, node.data);
        }
    ...
    size(node = this.root, count = 0) 
    {
        ...
        count = count + 1;
        count = node.size(node.left, count);
        count = node.size(node.right, count);
        return count
